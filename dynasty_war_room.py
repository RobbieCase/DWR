...
def compute_lineup(my_df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    if my_df.empty:
        return {k: my_df.head(0) for k in ["QB1","RB2","WR3","TE1","FLEX2","SFLX1","BENCH"]}
    mys = my_df.sort_values("rk")
    q = mys[mys["pos"]=="QB"]; rb = mys[mys["pos"]=="RB"]; wr = mys[mys["pos"]=="WR"]; te = mys[mys["pos"]=="TE"]
    take = lambda d, n: d.head(n)
    QB1 = take(q,1); RB2 = take(rb,2); WR3 = take(wr,3); TE1 = take(te,1)
    ...
    return {"QB1":QB1,"RB2":RB2,"WR3":WR3,"TE1":TE1,"FLEX2":FLEX2,"SFLX1":SFLX1,"BENCH":BENCH}
...
for key, default in {"queue": [], "off_board": [], "my_team": []}.items():
    if key not in st.session_state: st.session_state[key] = default

df["bucket"] = df["rk"].apply(bucket_label)

st.title("🏈 Dynasty Superflex Draft War Room")
st.caption("Live manage queue, drafted players, and your roster.")

f1, f2, f3, f4 = st.columns(4)
with f1: pos_filter = st.multiselect("Positions", ["QB","WR","RB","TE"], default=["QB","WR","RB","TE"])
with f2: max_age = st.slider("Max age", 18, 40, 40)
with f3: bucket_sel = st.multiselect("Buckets", ["Rounds 1–3","Rounds 4–6","Rounds 7–10","Rounds 11–15","Rounds 16+"], default=["Rounds 1–3","Rounds 4–6","Rounds 7–10","Rounds 11–15","Rounds 16+"])
with f4: name_query = st.text_input("Search player")

board = df.copy()
board = board[(board["pos"].isin(pos_filter)) & (board["age"].fillna(99) <= max_age) & (board["bucket"].isin(bucket_sel))]
if name_query.strip():
    s = name_query.strip().lower(); board = board[board["player"].str.lower().str.contains(s)]
...
my_counts = my_df["pos"].value_counts().to_dict() if not my_df.empty else {}
targets = {"QB": 3, "RB": 5, "WR": 7, "TE": 2}
dcols = st.columns(4)
for i, p in enumerate(["QB","RB","WR","TE"]):
    with dcols[i]:
        st.metric(label=f"{p} drafted", value=my_counts.get(p,0), delta=f"target {targets[p]}")
st.divider()
    ...
    for i, rec in enumerate(st.session_state.queue):
        ...
        with st.container(border=True):
            ...
            if b3.button("➕ My Team", key=f"add_{rec['player']}"):
                st.session_state.my_team.append({k: rec[k] for k in ["player","team","pos","age","rk","bucket"]})
            if b4.button("🚫 Drafted", key=f"off_{rec['player']}"):
                st.session_state.off_board.append({k: rec[k] for k in ["player","team","pos","age","rk","bucket"]})
...
with c2:
    ...
    if len(st.session_state.my_team)==0:
        st.info("No players yet. Use ➕ from the queue.")
    else:
        ...
        for idx, p in enumerate(st.session_state.my_team):
            if st.button(f"Remove {p['player']}", key=f"rm_team_{idx}"):
                st.session_state.my_team.pop(idx); st.rerun()
    ...
    if len(st.session_state.off_board)==0:
        ...
        for idx, p in enumerate(st.session_state.off_board):
            if st.button(f"Undo {p['player']}", key=f"rm_off_{idx}"):
                st.session_state.off_board.pop(idx); st.rerun()
...
if len(st.session_state.my_team)==0:
    ...
    with l1:
        st.markdown("**QB (1)**"); st.dataframe(lineup["QB1"], hide_index=True, use_container_width=True)
        st.markdown("**RB (2)**"); st.dataframe(lineup["RB2"], hide_index=True, use_container_width=True)
        st.markdown("**WR (3)**"); st.dataframe(lineup["WR3"], hide_index=True, use_container_width=True)
        st.markdown("**TE (1)**"); st.dataframe(lineup["TE1"], hide_index=True, use_container_width=True)
    with l2:
        st.markdown("**FLEX (2)** — best WR/RB/TE remaining"); st.dataframe(lineup["FLEX2"], hide_index=True, use_container_width=True)
        st.markdown("**SUPERFLEX (1)** — best remaining"); st.dataframe(lineup["SFLX1"], hide_index=True, use_container_width=True)
        st.markdown("**BENCH — remaining**"); st.dataframe(lineup["BENCH"], hide_index=True, use_container_width=True)
